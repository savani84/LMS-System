...
public class ViewApartment extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Rent> Rententries = new ArrayList<Rent>();
		...
		try
        {
            ...
            while( rs.next() )
            {
            	...
            	if(rs.getString("leasedate") != null){
            		try {
            			NeedDate = sdf.parse(rs.getString("leasedate"));
					} catch (ParseException e) {
						e.printStackTrace();
					}
            	}
            	Rententries.add(new Rent(rs.getString("custname"),rs.getInt("aptno"),rs.getString("leasename"),
            			NeedDate,rs.getString("doclist"),rs.getInt("deposit"),rs.getInt("rent"),rs.getInt("rent_id")));
            }
		...
		getServletContext().setAttribute( "Rententries", Rententries );
		request.getRequestDispatcher("Hw2/ViewApartment.jsp").forward(request, response);
	}
